...
describe('StandardModal', () => {
  ...
  it('closes on backdrop click by default', () => {
    const onClose = vi.fn()
    const { container } = render(<StandardModal {...defaultProps} onClose={onClose} />)
  ...
  it('does not close on backdrop click when closeOnBackdrop is false', () => {
    const onClose = vi.fn()
    render(<StandardModal {...defaultProps} onClose={onClose} closeOnBackdrop={false} />)
  ...
  it('does not close on Escape key when closeOnEscape is false', () => {
    const onClose = vi.fn()
    render(<StandardModal {...defaultProps} onClose={onClose} closeOnEscape={false} />)
  ...
  it('applies correct size classes', () => {
    const sizes: Array<'xs' | 'sm' | 'md' | 'lg' | 'xl' | 'large' | 'full'> = ['xs', 'sm', 'md', 'lg', 'xl', 'large', 'full']
    const expectedClasses = ['max-w-xs', 'max-w-sm', 'max-w-md', 'max-w-lg', 'max-w-xl', 'max-w-4xl', 'max-w-full']
    
    sizes.forEach((size, index) => {
      const { container } = render(<StandardModal {...defaultProps} size={size} />)
  ...
  it('applies correct position classes', () => {
    ...
    positions.forEach((position, index) => {
      const { container } = render(<StandardModal {...defaultProps} position={position} />)
  ...
  it('applies custom className', () => {
    const { container } = render(<StandardModal {...defaultProps} className="custom-modal" />)
  ...
  it('applies custom overlayClassName', () => {
    const { container } = render(<StandardModal {...defaultProps} overlayClassName="custom-overlay" />)
