...
const ValidationSummary: React.FC<ValidationSummaryProps> = ({
  ...
  return (
    <div className={`p-3 rounded-md border ${severityInfo.bgColor} ${severityInfo.borderColor} ${className}`}>
        ...
        <div className="flex-1">
          <div className={`text-sm font-medium ${severityInfo.textColor}`}>
            {hasErrors && (
              <span>
                ...
                {hasWarnings && `, ${warningCount} ${warningCount === 1 ? 'warning' : 'warnings'}`}
              </span>
            )}
          ...
          {showDetails && (
            <div className="mt-2 space-y-1">
              {errors.length > 0 && (
                <div>
                  <div className={`text-xs font-medium ${severityInfo.textColor} mb-1`}>
                    Errors:
                  </div>
                  <ul className={`text-xs ${severityInfo.textColor} space-y-0.5 pl-2`}>
