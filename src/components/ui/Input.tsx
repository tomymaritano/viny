...
const inputVariants = cva(
  'flex w-full rounded-xl border-2 bg-transparent px-3 py-4 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-300',
  {
    variants: {
      variant: {
        default:
          'border-theme-text-secondary/20 bg-theme-bg-secondary/30 focus-visible:border-theme-accent-primary focus-visible:bg-theme-accent-primary/5',
        glassmorphism:
          'border-theme-text-secondary/20 backdrop-blur-xl bg-theme-bg-secondary/20 focus-visible:border-theme-accent-primary focus-visible:bg-theme-accent-primary/5 focus-visible:backdrop-blur-sm',
        error:
          'border-red-400 bg-red-50/10 text-red-600 focus-visible:border-red-500',
        success:
          'border-green-400 bg-green-50/10 text-green-600 focus-visible:border-green-500',
      },
      size: {
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  (
    { className, variant, size, startIcon, endIcon, error, success, ...props },
    ref
  ) => {
