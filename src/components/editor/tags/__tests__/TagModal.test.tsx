...
vi.mock('../../../Icons', () => ({
  default: {
    ...
    Plus: ({ size }: { size?: number }) => (
      <div data-testid="plus-icon" data-size={size}>
        Plus
      </div>
    ),
  ...
  default: ({ isOpen, onClose, tagName }: any) => {
    ...
    return (
      <div data-testid="tag-settings-modal">
        ...
        <button onClick={onClose} data-testid="close-tag-settings">
          Close Settings
        </button>
      </div>
    )
  ...
  getAvailableTagColors: vi.fn(() => [
    {
      key: 'ocean',
      name: 'Ocean',
      preview: { bg: '#0891B2', border: '#0E7490' },
    },
    {
      key: 'forest',
      name: 'Forest',
      preview: { bg: '#059669', border: '#047857' },
    },
    {
      key: 'sunset',
      name: 'Sunset',
      preview: { bg: '#EA580C', border: '#C2410C' },
    },
    {
      key: 'lavender',
      name: 'Lavender',
      preview: { bg: '#7C3AED', border: '#6D28D9' },
    },
  ]),
}))
...
describe('TagModal', () => {
  ...
  describe('Global Mode', () => {
    it('should render create tag section in global mode', () => {
      ...
      expect(
        screen.getByPlaceholderText('Type new tag name...')
      ).toBeInTheDocument()
  ...
  describe('Tag Deletion', () => {
    ...
    it('should call removeTagFromAllNotes when delete is confirmed', () => {
      ...
      expect(mockShowSuccess).toHaveBeenCalledWith(
        'Tag "tag1" removed from all notes'
      )
    })
    ...
    it('should show error message when deletion fails', () => {
      ...
      expect(mockShowError).toHaveBeenCalledWith(
        'Failed to remove tag "tag1". Please try again.'
      )
    })
    ...
    it('should handle tag deletion for any tag name', () => {
      ...
      expect(mockRemoveTagFromAllNotes).toHaveBeenCalledWith(
        ' tag-with-spaces '
      )
    })
  })
  ...
  describe('Edge Cases', () => {
    it('should handle empty available tags list', () => {
      ...
      expect(
        screen.queryByText('Choose from existing tags')
    ...
    it('should show message when no tags exist in global mode', () => {
      ...
      expect(
        screen.getByText('No tags exist yet. Create your first tag above.')
      ).toBeInTheDocument()
    })
