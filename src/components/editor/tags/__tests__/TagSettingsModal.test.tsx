...
vi.mock('../../../ui/CustomTag', () => ({
  default: ({ name, color }: { name: string; color: string }) => (
    <div data-testid="custom-tag" data-color={color}>{name}</div>
  )
}))

vi.mock('../../../../utils/customTagColors', () => ({
  getAvailableTagColors: vi.fn(() => [
    { key: 'blue', name: 'Blue', preview: { bg: '#3B82F6', border: '#2563EB' } },
    { key: 'green', name: 'Green', preview: { bg: '#10B981', border: '#059669' } },
    { key: 'red', name: 'Red', preview: { bg: '#EF4444', border: '#DC2626' } },
    { key: 'yellow', name: 'Yellow', preview: { bg: '#F59E0B', border: '#D97706' } }
  ])
}))
...
describe('TagSettingsModal', () => {
  ...
  it('calls onTagNameChange when Save is clicked with changed name', () => {
    ...
    render(<TagSettingsModal {...defaultProps} onTagNameChange={onTagNameChange} />)
  ...
  it('calls onClose after saving', () => {
    const onClose = vi.fn()
    const onTagNameChange = vi.fn()
    render(<TagSettingsModal {...defaultProps} onClose={onClose} onTagNameChange={onTagNameChange} />)
