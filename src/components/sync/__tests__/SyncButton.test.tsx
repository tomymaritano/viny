...
vi.mock('lucide-react', () => ({
  RefreshCw: ({ size, className }: any) => (
    <div data-testid="refresh-icon" className={className} data-size={size}>Refresh</div>
  ),
  AlertTriangle: ({ size, className }: any) => (
    <div data-testid="alert-icon" className={className} data-size={size}>Alert</div>
  ),
  Cloud: ({ size }: any) => <div data-testid="cloud-icon" data-size={size}>Cloud</div>,
  WifiOff: ({ size, className }: any) => (
    <div data-testid="wifi-off-icon" className={className} data-size={size}>Offline</div>
  )
}))
...
describe('SyncButton', () => {
  ...
  it('handles conflict resolution', () => {
    ...
    expect(mockUseSync.resolveConflict).toHaveBeenCalledWith(
      'conflict-1',
      { strategy: 'merge' }
    )
