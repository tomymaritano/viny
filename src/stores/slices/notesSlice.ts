...
export const createNotesSlice: StateCreator<NotesSlice, [], [], NotesSlice> = (
  ...
  return {
    ...
    loadNotes: async () => {
      const notes = await withErrorHandling(() => repository.getNotes(), 'load')
        ...
        const sortedNotes = [...state.notes].sort((a, b) => {
          ...
          switch (state.sortBy) {
            ...
            case 'updated':
              ...
              break
            case 'notebook':
              comparison = a.notebook
                .toLowerCase()
                .localeCompare(b.notebook.toLowerCase())
              break
          }
    ...
    removeNote: async noteId => {
      ...
      if (success) {
        ...
        set(state => ({
          ...
          isEditorOpen:
            state.currentNote?.id === noteId ? false : state.isEditorOpen,
        }))
    ...
    removeTagFromAllNotes: async tagName => {
      ...
      const updatedCurrentNote =
        state.currentNote && state.currentNote.tags?.includes(tagName)
          ? {
              ...state.currentNote,
    ...
    renameTagInAllNotes: async (oldTagName, newTagName) => {
      ...
      // Update current note if it was affected
      const updatedCurrentNote =
        state.currentNote && state.currentNote.tags?.includes(oldTagName)
          ? {
              ...state.currentNote,
              tags: state.currentNote.tags.map(tag =>
      ...
      if (success) {
        ...
        logger.debug(
          `Tag "${oldTagName}" renamed to "${newTagName}" in ${modifiedCount} notes`
        )
      }
