...
describe('useStorageInit', () => {
  ...
  const mockNotebooks = [
    { name: 'test', color: '#000000', count: 1, isDefault: false },
  ]
  ...
  describe('Error Handling', () => {
    ...
    it('should continue loading tag colors even if notes fail', async () => {
      vi.mocked(StorageService.getInstance().loadNotes).mockRejectedValue(
        new Error('Failed')
      )
    ...
    it('should handle tag colors loading error', async () => {
      vi.mocked(StorageService.getInstance().loadTagColors).mockRejectedValue(
        new Error('Failed')
      )
  ...
  describe('Recovery', () => {
    it('should attempt recovery on critical error', async () => {
      ...
      if (criticalError.message.includes('QUOTA_EXCEEDED')) {
        expect(
          StorageService.getInstance().recoverFromBackup
        ).toHaveBeenCalled()
      }
    })
  ...
  describe('Data Validation', () => {
    ...
    it('should validate tag colors', async () => {
      ...
      vi.mocked(StorageService.getInstance().loadTagColors).mockResolvedValue(
        invalidTagColors as any
      )
  ...
  describe('Performance', () => {
    it('should load data in parallel', async () => {
      ...
      const loadTagColorsSpy = vi.mocked(
        StorageService.getInstance().loadTagColors
      )
    ...
    it('should not re-initialize on re-render', () => {
      ...
      const initialCalls = vi.mocked(StorageService.getInstance().loadNotes)
        .mock.calls.length
      ...
      expect(
        vi.mocked(StorageService.getInstance().loadNotes)
