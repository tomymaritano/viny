...
const createMouseEvent = (
  ...
  const event = {
    ...
    stopPropagation:
      options.stopPropagation !== false ? stopPropagation : vi.fn(),
  } as any
...
describe('useSidebarState', () => {
  ...
  describe('Notebook context menu', () => {
    ...
    it('should update notebook context menu with different notebook', () => {
      ...
      act(() => {
        result.current.handleNotebookRightClick(
  ...
  describe('Context menu management', () => {
    ...
    it('should preserve other state when closing context menus', () => {
      ...
      act(() => {
        result.current.handleTagRightClick(
          createMouseEvent(100, 100),
          'preserved-tag'
        )
      })
  ...
  describe('Method stability', () => {
    it('should provide stable method references', () => {
      ...
      expect(result.current.handleTagRightClick).toBe(
        initialMethods.handleTagRightClick
      )
      expect(result.current.handleNotebookRightClick).toBe(
        initialMethods.handleNotebookRightClick
      )
      expect(result.current.handleTrashRightClick).toBe(
        initialMethods.handleTrashRightClick
      )
      expect(result.current.closeAllContextMenus).toBe(
        initialMethods.closeAllContextMenus
      )
      expect(result.current.toggleNotebookExpansion).toBe(
        initialMethods.toggleNotebookExpansion
      )
      expect(result.current.startEditingNotebook).toBe(
        initialMethods.startEditingNotebook
      )
      expect(result.current.cancelEditingNotebook).toBe(
        initialMethods.cancelEditingNotebook
      )
    })
  })
