...
const mockSuggestions = ['javascript', 'typescript', 'react', 'testing', 'hooks', 'frontend', 'backend']
...
describe('useTagManager', () => {
  beforeEach(() => {
    ...
    vi.mocked(tagValidation.addTag).mockImplementation((tags, newTag) => [...tags, newTag])
    vi.mocked(tagValidation.removeTag).mockImplementation((tags, index) => tags.filter((_, i) => i !== index))
    vi.mocked(tagValidation.updateTag).mockImplementation((tags, index, newTag) => {
      const newTags = [...tags]
  ...
  describe('Hook initialization', () => {
    it('should initialize with default state', () => {
      const { result } = renderHook(() => useTagManager(testNote, mockOnTagsChange))
  ...
  describe('Tag suggestions', () => {
    ...
    it('should exclude existing tags from suggestions', () => {
      ...
      const { result } = renderHook(() => useTagManager(noteWithTags, mockOnTagsChange))
  ...
  describe('Adding tags', () => {
    it('should add a new tag', () => {
      ...
      expect(vi.mocked(tagValidation.addTag)).toHaveBeenCalledWith(['existing-tag', 'another-tag'], 'new-tag')
      expect(mockOnTagsChange).toHaveBeenCalledWith(['existing-tag', 'another-tag', 'new-tag'])
      expect(result.current.tagInput).toBe('')
      expect(result.current.showTagSuggestions).toBe(false)
      expect(result.current.selectedSuggestionIndex).toBe(-1)
      expect(vi.mocked(logger).debug).toHaveBeenCalledWith('Tag added successfully:', 'new-tag')
    })
    ...
    it('should handle errors when adding tags', () => {
      ...
      expect(vi.mocked(logger).error).toHaveBeenCalledWith('Failed to add tag:', expect.any(Error))
      expect(mockOnTagsChange).not.toHaveBeenCalled()
    })
  })
  ...
  describe('Removing tags', () => {
    it('should remove a tag by index', () => {
      ...
      expect(vi.mocked(tagValidation.removeTag)).toHaveBeenCalledWith(['existing-tag', 'another-tag'], 0)
      expect(mockOnTagsChange).toHaveBeenCalled()
      expect(vi.mocked(logger).debug).toHaveBeenCalledWith('Tag removed at index:', 0)
    })
    ...
    it('should handle errors when removing tags', () => {
      ...
      expect(vi.mocked(logger).error).toHaveBeenCalledWith('Failed to remove tag:', expect.any(Error))
      expect(mockOnTagsChange).not.toHaveBeenCalled()
    })
  })
  ...
  describe('Updating tags', () => {
    it('should update a tag by index', () => {
      ...
      expect(vi.mocked(tagValidation.updateTag)).toHaveBeenCalledWith(['existing-tag', 'another-tag'], 0, 'updated-tag')
      expect(mockOnTagsChange).toHaveBeenCalled()
      expect(vi.mocked(logger).debug).toHaveBeenCalledWith('Tag updated at index:', 0, 'to:', 'updated-tag')
    })
    ...
    it('should handle errors when updating tags', () => {
      ...
      expect(vi.mocked(logger).error).toHaveBeenCalledWith('Failed to update tag:', expect.any(Error))
      expect(mockOnTagsChange).not.toHaveBeenCalled()
    })
  })
  ...
  describe('Keyboard navigation', () => {
    ...
    it('should add selected suggestion on Enter key', () => {
      ...
      expect(vi.mocked(tagValidation.addTag)).toHaveBeenCalledWith(['existing-tag', 'another-tag'], 'javascript')
    })
    ...
    it('should wrap around when navigating past last suggestion', () => {
      ...
      act(() => {
        result.current.setTagInput('e') // Get some suggestions
        result.current.setSelectedSuggestionIndex(result.current.filteredSuggestions.length - 1)
      })
    ...
    it('should wrap around when navigating before first suggestion', () => {
      ...
      expect(result.current.selectedSuggestionIndex).toBe(result.current.filteredSuggestions.length - 1)
    })
  ...
  describe('Context menu', () => {
    ...
    it('should handle edit action from context menu', () => {
      ...
      expect(mockHandleTagEdit).toHaveBeenCalledWith('test-tag', 0, expect.any(Function))
    ...
    it('should handle remove action from context menu', () => {
      ...
      expect(vi.mocked(tagValidation.removeTag)).toHaveBeenCalledWith(['existing-tag', 'another-tag'], 1)
      expect(result.current.contextMenu.show).toBe(false)
    })
  ...
  describe('Edge cases', () => {
    it('should handle note with no tags array', () => {
      ...
      const { result } = renderHook(() => useTagManager(noteWithoutTags, mockOnTagsChange))
      ...
      expect(vi.mocked(tagValidation.addTag)).toHaveBeenCalledWith([], 'new-tag')
    })

    it('should handle note with null tags', () => {
      const noteWithNullTags = { ...testNote, tags: null }
      const { result } = renderHook(() => useTagManager(noteWithNullTags as any, mockOnTagsChange))
