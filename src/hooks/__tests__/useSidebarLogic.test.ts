...
// Mock data
const mockNotes = [
  { id: '1', title: 'Note 1', isTrashed: false, isPinned: true, status: 'active', tags: ['tag1', 'tag2'] },
  { id: '2', title: 'Note 2', isTrashed: false, isPinned: false, status: 'draft', tags: ['tag1', 'tag3'] },
  { id: '3', title: 'Note 3', isTrashed: false, isPinned: false, status: 'in-progress', tags: ['tag2'] },
  { id: '4', title: 'Note 4', isTrashed: false, isPinned: false, status: 'review', tags: ['tag3'] },
  { id: '5', title: 'Note 5', isTrashed: false, isPinned: false, status: 'completed', tags: [] },
  { id: '6', title: 'Note 6', isTrashed: false, isPinned: false, status: 'archived', tags: ['tag1'] },
  { id: '7', title: 'Note 7', isTrashed: true, isPinned: false, status: 'active', tags: [] },
]
...
vi.mock('../../utils/statsUtils', () => ({
  getStats: vi.fn((notes) => ({
    ...
    byStatus: {
      ...
      'in-progress': notes.filter(n => !n.isTrashed && n.status === 'in-progress').length,
      review: notes.filter(n => !n.isTrashed && n.status === 'review').length,
      completed: notes.filter(n => !n.isTrashed && n.status === 'completed').length,
      archived: notes.filter(n => !n.isTrashed && n.status === 'archived').length,
    }
  }))
}))
...
describe('useSidebarLogic', () => {
  ...
  describe('Tags calculation', () => {
    ...
    it('should exclude tags from completed and archived notes', () => {
      ...
      const tag1Count = result.current.tagsWithCounts.find(t => t.tag === 'tag1')?.count
  ...
  describe('Notebook actions delegation', () => {
    it('should delegate notebook actions correctly', () => {
      ...
      expect(mockUpdateNotebook).toHaveBeenCalledWith('nb1', { name: 'Updated' })
  ...
  describe('Method stability', () => {
    it('should provide stable method references', () => {
      ...
      expect(result.current.handleSectionClick).toBe(initialMethods.handleSectionClick)
      expect(result.current.handleToggleSection).toBe(initialMethods.handleToggleSection)
      expect(result.current.handleSettingsClick).toBe(initialMethods.handleSettingsClick)
    })
  })
})