import { type Notebook, NOTEBOOK_VALIDATION } from '../types/notebook'
...
export const validateNotebookName = (
  ...
  if (trimmed.length < NOTEBOOK_VALIDATION.minLength) {
    return {
      isValid: false,
      error: `Category name must be at least ${NOTEBOOK_VALIDATION.minLength} characters`,
    }
  }
  ...
  if (trimmed.length > NOTEBOOK_VALIDATION.maxLength) {
    return {
      isValid: false,
      error: `Category name must be less than ${NOTEBOOK_VALIDATION.maxLength} characters`,
    }
  }
  ...
  if (
    NOTEBOOK_VALIDATION.reservedNames.some(
      reserved => reserved.toLowerCase() === trimmed.toLowerCase()
    )
  ) {
    return {
      isValid: false,
      error: 'This is a reserved name. Please choose a different name',
    }
  }

  // Check duplicates (exclude current notebook when editing)
  const duplicateExists = existingNotebooks.some(
    notebook =>
...
export const validateNotebookNesting = (
  ...
  if (parent.level >= NOTEBOOK_VALIDATION.maxNestingLevel) {
    return {
      isValid: false,
      error: `Maximum nesting level is ${NOTEBOOK_VALIDATION.maxNestingLevel}`,
    }
  }
...
export const validateNotebookMove = (
  ...
  if (isMovingToChild(newParentId)) {
    return {
      isValid: false,
      error: 'Cannot move category to one of its subcategories',
    }
  }
