...
export const applyTypographySettings = (settings: {
  ...
  const previewFontSize =
    settings.previewFontSize || settings.editorFontSize || 14
  ...
  const previewLineHeight =
    settings.previewLineHeight || settings.lineHeight || 1.6
  ...
  // Apply preview theme
  // If no preview theme is set, use a default based on the main theme
  let previewTheme = settings.previewTheme
  if (!previewTheme || previewTheme === 'default') {
    const resolvedTheme = resolveSystemTheme(settings.theme || 'dark')
    previewTheme = resolvedTheme === 'dark' ? 'default-dark' : 'default-light'
  }
 ...
 */
export const applyThemeCompletely = (
  theme: ThemeValue,
  options?: {
